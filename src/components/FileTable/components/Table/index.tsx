...
const Table: React.FC<Props> = (props) => {
  let setList = [
    ...
    },
    {
      ...
      callback(index: any) {
        ...
        let shareKey = userKey?.substring(0, 8);
        if (typeof index == 'number') {
          console.log(data?.list[index]);
          let sid = data?.list[index].sid;
        ...
        (async () => {
          const shareUrl = await contract.file_share({
            share_key: shareKey,
            files_share: shareFile,
          });
          const time = new Date().getTime();
          props.setShareUrl(
            `http://172.18.1.174:8000/share?shareKey=${shareKey}&ID=${getID}&shareTime=${time}`,
          );
        })();
  ...
  function showCheck(index: number, setItem?: any) {
    let item = setItem || data?.list[index];
    setCheckRender(<Check item={item} title={props.title} setCheckRender={setCheckRender}></Check>);
  }
  useEffect(() => {
    props.setShowCheck(() => {
      return showCheck;
      ...
      let results = await Promise.all(list.map(async(items:any)=>{
        ...
        if(result.deals.length!=0){
          result.deals.forEach((res:any)=>{            
            storageProvidersArr.push(res.storageProvider)
          })
        }else{
    ...
    return data?.list?.map((item, index) => {
      let trRender = (item: any) => {
        ...
        return props.headerList.map((headerItem) => {
          ...
          if (headerItem.key === 'sid') {
            r = (
              <Popover content={item[headerItem.key]}>
                <div className={styles.keyBox} onClick={() => goCheck(index)}>
                  {item[headerItem.key]}
                </div>
              </Popover>
            );
        ...
        <div
          ...
          onDoubleClick={() => {
            ...
            if (item.file_folder[0] !== 'music') {
              console.log(getCidUrl(item.cid,item.file_name));
  ...
  function render() {
    ...
    if (!data?.nextPage && !data?.list.length) {
